...
#define GFX_THROW_NO_INFO(hrcall) if (HRESULT hr = (hrcall); FAILED(hr)) throw GFX_EXCEPT_NO_INFO(hr) 
...
#define GFX_THROW_INFO(hrcall) INFOMAN_SET; if (HRESULT hr = (hrcall); hr != S_OK) throw GFX_EXCEPT(hr)
...
#define GFX_THROW_INFO_ONLY(hrcall) INFOMAN_SET; (hrcall); if (auto v = INFOMAN_MSGS; !v.empty()) throw Graphics::InfoException{ __LINE__, WFILE, v }
